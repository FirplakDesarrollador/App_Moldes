...
// Wait, both URLs/keys seem to be the same in the source? 
// No, createClientTH often points to a different project.

async function run() {
    console.log("--- Checking planta_moldes in both clients ---")
    const { data: d1 } = await supabaseRegular.from('planta_moldes').select('*').limit(1)
    console.log("Regular client:", d1?.length || 0)
}
run()
